...
$this->title = 'Менеджер групповых программ';
        ...
        <?= GridView::widget([
            ...
            'columns' => [
                ...
                [
                    ...
                    'header' => 'Действия',
                    'template' => '<ul class="panel-icons clearfix">{update} {delete} {up} {down} {lessons}</ul>',
                    'buttons' => [
                        ...
                        'up' => function ($url, $model) {
                            if($model->id != 1 && $model->id != 2 && $model->position != 30) {
                        ...
                        'lessons' => function ($url, $model) {
                            $url = Url::to(['/program-classes', 'id' => $model->id]);
                            return '<li>' . Html::a('Занятия', $url, ['class' => 'btn btn--grey', 'data-pjax' => '0']) . '</li>';
                        },
                    ]
                ],
            ],
...
$this->registerJs(<<<JS
	jQuery('td').click(function (e) {
        ...
        if(e.target == this && id != undefined)
            location.href = '/admin/group-programs/update?id=' + id;
    });
JS
);
